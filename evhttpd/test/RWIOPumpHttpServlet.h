/* 
 * File:   RWIOPumpHttpServlet.h
 * Author: try
 *
 * Created on 2011年7月15日, 下午11:47
 */

#ifndef RWIOPUMPHTTPSERVLET_H
#define	RWIOPUMPHTTPSERVLET_H
...
/**
 * 使用RWIOPump来下载文件实例，用法
 * 
 * http://192.168.99.60:3080/rw/home/vdev/flvs/1.flv?speed=1048576
 */
class RWIOPumpHttpServlet : public HttpServlet, public IEventListener{
public:
    RWIOPumpHttpServlet():flvfd(-1), ioPump(NULL), ioReader(NULL), ioWriter(NULL){}
    virtual ~RWIOPumpHttpServlet(){
        close();
        if(ioPump){
            delete ioPump;
            ioPump = NULL;
    ...
    virtual void service(Request& req, Response& resp){
        ...
        ioPump = new RWIOPump(ioReader, ioWriter, speedval, SEND_SIZE);
